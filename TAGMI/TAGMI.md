...
TAGMI lets users create a wager on the outcome of any event, and tokenize the positions on either side of the bet as tradeable bearer assets.

Using TAGMI, any two parties can create a bet on the outcome of any given event and designate an arbitrator that will decide who won the bet. The two parties can bet any ERC-20 tokens, in any amounts, and do not have to bet the same tokens or the same amounts against one another.

TAGMI will then issue NFTs to the two betting parties that represent their stakes in the wager. These NFTs are what we refer to as TAGs (short for Tokenized wAGers).

These TAGs can be bought/sold/transferred to any other wallet. When a new party takes ownership of a TAG, they will receive payment in the event their position in the bet wins.
...
Initially, I ran the contracts through MythX, which resulted in 1 low vunlerability being detected, included below. I then pursued a comprehensive deep dive. The following number of issues were found, categorized by their severity:
